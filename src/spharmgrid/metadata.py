...
def _laplacian_units(
    units: str, operation: Literal["laplacian", "inverse_laplacian"]
) -> str:
    """Apply only the unit suffix changes generated by spharmgrid operators."""
    if operation == "laplacian":
        return units.removesuffix(" m2") if units.endswith(" m2") else f"{units} m-2"
    return units.removesuffix(" m-2") if units.endswith(" m-2") else f"{units} m2"
