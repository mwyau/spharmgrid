...
- `spharmgrid[torch]` — tensor-native PyTorch operations backed by `torch-harmonics`;
...
For differentiable tensor workflows, install `spharmgrid[torch]` and use the
separate `spharmgrid.torch` API:
