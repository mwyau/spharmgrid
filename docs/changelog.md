...
- Added the optional `spharmgrid.jax` array API backed by S2FFT, covering the
  same 19 scientific operations as `spharmgrid.torch`: spherical harmonic
  filtering, scalar and vector regridding, differential operators, atmospheric
  kinematics, Helmholtz decomposition, and inverse wind transforms.
- Added GL and CC/MWSS support for JAX arrays, including leading dimensions,
  `jit`, `vmap`, and automatic differentiation. The JAX API uses x64 mode
  with `float64` spatial inputs.
- S2FFT Price–McEwen precomputations are cached across transforms. Scalar
  transforms use the real-field path and spin-1 transforms use the complex
  path.
...
- Added numerical JAX-to-DUCC comparisons across GL and CC/MWSS grids, scalar
  and vector operations, regridding, differential operators, and atmospheric
  wind diagnostics.
- Expanded optional-backend tests for `jit`, `vmap`, automatic
  differentiation, spectral-edge cases, transform precomputation, and Xarray
  `DataArray` and `Dataset` PyTrees through `gdm-xarray-jax`.
- Updated CI to test JAX, PyTorch, and pyspharm parity in compatible dependency
  environments and to exercise the supported optional-backend dependency
  floors.
