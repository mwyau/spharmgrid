...
`.github/workflows/ncl-parity.yml` workflow runs only through
`workflow_dispatch` and generates all numerical data for each run. No
NCL-generated numerical data is stored in Git.
...
- random scalar and wind fields from `np.random.default_rng(42)`.

Python writes the input arrays once. NCL and spharmgrid use the same arrays.
The random seed is stored with the input and copied to the NCL output. The
normalizer checks the coordinates and converts NCL output to ascending latitude
...
NCL `exp_tapersh` uses
`S(l) = exp(-[l(l+1)/(N(N+1))]^2)`. Solving for the NCL mode gives
`N ≈ 34.002499526`. The tests compare the NCL taper weights with the
spharmgrid expression before comparing spatial fields.
